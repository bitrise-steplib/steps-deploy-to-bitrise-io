...
func main() {
	...
	// deployTestResults is called even when the Test Reports API token is empty
	// so it can warn the user when test results are present but cannot be
	// uploaded, instead of silently skipping the upload.
...
func deployTestResults(config Config, logger loggerV2.Logger) {
	...
	if config.AddonAPIToken == "" {
		logger.Println()
		logger.Warnf("Test results were found but they will not be uploaded to the Tests tab: the Test Reports API token (addon_api_token) is not set.")
		logger.Warnf("This most commonly happens on builds triggered by a pull request, where this token is not available by default.")
		logger.Warnf("To upload test results from such builds, trigger the build from a push event instead.")
		return
	}
