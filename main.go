...
func deployTestResults(config Config, logger loggerV2.Logger) {
	...
	for i, result := range testResults {
		...
		if len(result.Attachments) > 0 {
			logger.Printf("- %s (generated by the %s) with %d attachment(s):", result.Name, stepNameWithIndex(result.StepInfo), len(result.Attachments))
			for _, pth := range result.Attachments {
				logger.Printf("  - %s", pth)
			}
		} else {
